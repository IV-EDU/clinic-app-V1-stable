...
def _view_mode() -> str:
    ...
    return raw if raw in {"payments", "treatments"} else "payments"
...
def collections_day(d):
    ...
    cur = conn.cursor()
    params: list[str] = [d]
    if view_mode == "treatments":
        query = """
            SELECT
              CASE
                WHEN COALESCE(pay.parent_payment_id, '') = '' THEN pay.id
                ELSE pay.parent_payment_id
              END AS treatment_id,
              pay.patient_id,
              p.full_name,
              COALESCE(tr.treatment, '') AS treatment,
              COALESCE(SUM(pay.amount_cents), 0) AS amount_cents,
              COUNT(*) AS payments_count
            FROM payments pay
            JOIN patients p ON p.id=pay.patient_id
            LEFT JOIN payments tr
              ON tr.id = CASE
                           WHEN COALESCE(pay.parent_payment_id, '') = '' THEN pay.id
                           ELSE pay.parent_payment_id
                         END
             AND tr.patient_id = pay.patient_id
        ...
        if doctor_id:
            if doctor_id == ANY_DOCTOR_ID:
                ...
                params.append(doctor_id)
        query += """
            GROUP BY treatment_id, pay.patient_id, p.full_name, tr.treatment
        ...
        return jsonify(
            {
                ...
                "view_mode": view_mode,
                "payments_count": len(rows_data) if view_mode == "payments" else None,
                "treatments_count": len(rows_data) if view_mode == "treatments" else None,
                "rows": [
                    {
