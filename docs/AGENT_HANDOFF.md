...
- Reception Phase 1 is partially live now:
  ...
  - Treatment-card `edit_payment` drafts can now be created, reviewed, returned, edited/resubmitted, and approved onto the same live payment
  - Treatment-card `edit_treatment` drafts can now be created, reviewed, returned, edited/resubmitted, and approved onto the same live treatment
  - Narrow approval exists for:
    ...
    - Treatment-card locked `edit_payment` drafts
    - Treatment-card locked `edit_treatment` drafts
- Approval currently supports only:
  - creating a new live patient + treatment for Desk-origin `new_treatment`
  - attaching a Desk-origin `new_treatment` draft to one manager-chosen existing patient without editing that patient profile
  - posting one live child payment onto the locked treatment for treatment-card `new_payment`
  - updating the locked live payment for treatment-card `edit_payment`
  - updating the locked live treatment for treatment-card `edit_treatment`
...
- Do not casually widen approval to desk-origin payment drafts, patient reassignment beyond manager-chosen existing-patient treatment routing, or duplicate-resolution flows without updating the Reception planning docs first.
