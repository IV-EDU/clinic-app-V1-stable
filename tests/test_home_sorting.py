...
def test_patient_open_link_carries_return_to_and_back_restores_page_sort(logged_in_client):
    ...
    assert "return_to=/" in patient_href
    ...
    back_href = re.search(r'class="btn secondary back-btn" href="([^"]+)"', detail_html)
...
def test_patient_back_uses_remembered_return_after_detail_without_return_to(logged_in_client):
    ...
    first_detail = logged_in_client.get(f"/patients/{pid}?return_to=%2F%3Fpage%3D3%26sort%3Dold")
    ...
    back_href = re.search(r'class="btn secondary back-btn" href="([^"]+)"', plain_html)
    assert back_href, "Expected back button in patient detail"
...
def test_patient_detail_rejects_external_return_to(logged_in_client):
    ...
    assert detail.status_code == 200
    back_href = re.search(r'class="btn secondary back-btn" href="([^"]+)"', html)
    assert back_href, "Expected back button in patient detail"
